...
extension String {
	...
	public func componentsSeparatedByString(_ token: Character) -> [String] {
		return self.split(maxSplits: Int.max, omittingEmptySubsequences: false) { $0 == token }.map { String($0) }
	}
...
public func sequence(_ ms: [String]) -> [String] {
	return sequence(ms.map { m in m.map{$0} }).map(String.init(describing:))
}
